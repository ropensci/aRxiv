...
- Each field is a single character string. `authors`, `link_doi`, and
  ...
  (`|`).
- Missing entries are simply an empty character string (`""`).
...
Speed vs Terence P. Speed). Further, arXiv has no ability to
distinguish multiple individuals with the same name (cf
[ORCID](http://orcid.org)).

Authors' institutional affiliations are mostly missing.
The arXiv submission form does not include an affiliation field;
affiliations are entered within the author field, in parentheses.
The [instructions](http://arxiv.org/help/prep#author)) may not be
widely read.
...
a sense of how many records you will receive. If the count is quite
large, you may wish to refine your query.

[arXiv](http://arxiv.org) has a hard limit of around 50,000 records;
for a query that matches more than 50,000 manuscripts, there is no way
to receive the full results.

The simplest solution to this problem is to break the query into
smaller pieces, for example using slices of time, with a range of
dates for `submittedDate` or `lastUpdatedDate`.
