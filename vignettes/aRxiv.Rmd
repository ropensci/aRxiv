...
```{r, install_from_runiverse, eval=FALSE}
...
We can open the abstract pages for these manuscripts
...
to get the full results. (There's also `lastUpdatedDate`, but it seems
we can no longer search on that field.)

The date/time information is of the form `YYYYMMDDHHMM`, for
example `200710181225` for `2007-10-18 12:25`, but you need to provide
ranges in square brackets, like `[20071018 TO 20071020]`.
For example, to get all manuscripts with initial submission on 2007-10-18:
...
```{r dates_range_notimes}
arxiv_count('submittedDate:[20071018 TO 20071018]')
```

If you give a partial date, it's treated as the earliest date/time
that matches for the beginning of the range, and the latest date/time
that matches for the end of the range.
